...
- (UITextView *)textView {
    if (_textView == nil) {
        _textView = [[UITextView alloc] initWithFrame:CGRectMake(5, 60, self.viewSize.width - 10, self.viewSize.height - 65)];
...
- (UITextView *)filterTextView {
    if (_filterTextView == nil) {
        _filterTextView = [[UITextView alloc] initWithFrame:CGRectMake(5, 60, self.viewSize.width - 10, self.viewSize.height - 65)];
...
- (void)foldBtnAction:(UIButton *)sender {
    ...
    if (sender.selected) {
        [UIView animateWithDuration:0.3 animations:^{
            self.view.frame = CGRectMake(self.viewOrigin.x, self.viewOrigin.y, self.viewSize.width, 60);
...
- (void)filterBtnAction:(UIButton *)sender {
    ...
    [self.sideMenu loadMenuWithItems:menuItems menuWidth:70 origin:CGPointMake([self xFromLeftWithIndex:4] - 10, TopOfButtons)];
